...
<head>
    ...
    <title>Billetterie — RFC Liège</title>
...
<section class="page-banner">
    ...
    <div class="page-banner-content">
        ...
        <h1 class="page-h1">Billetterie</h1>
        <p class="page-desc">Achetez vos billets pour les prochains matchs du RFC Liège.</p>
    </div>
</section>
...
<?php if($cms_r&&$cms_r["contenu"]): ?><main class="page-main"><div class="page-main-inner cms-content"><?= cms_render($cms_r["contenu"]) ?></div></main><?php else: ?><main class="page-main">
    <div class="page-main-inner">
        ...
        <section class="content-section">
            ...
            <?php
            try { $upcoming = $pdo->query("SELECT * FROM matches WHERE date_match >= NOW() ORDER BY date_match ASC LIMIT 5")->fetchAll(); }
            catch(PDOException $e){ $upcoming = []; }
            ?>
            <?php if(!empty($upcoming)): ?>
            <div style="display:flex;flex-direction:column;gap:10px;">
            <?php foreach($upcoming as $um): ?>
                <div style="background:#fff;border:1px solid #EBEBEB;border-left:5px solid var(--rouge);border-radius:10px;padding:20px 24px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px;">
                    <div>
                        <div style="font-family:'Bebas Neue',sans-serif;font-size:1.4rem;color:var(--navy);">RFC Liège <span style="color:var(--rouge);">vs</span> <?= htmlspecialchars($um['equipe_adversaire']) ?></div>
                        <div style="font-size:0.84rem;color:#888;margin-top:4px;">📅 <?= (new DateTime($um['date_match']))->format('d/m/Y') ?> · 🕐 <?= (new DateTime($um['date_match']))->format('H:i') ?> · 📍 <?= htmlspecialchars($um['stade']) ?><?= !empty($um['competition'])?' · 🏆 '.htmlspecialchars($um['competition']):'' ?></div>
                    </div>
                    <a href="#" class="btn-primary" style="font-size:0.84rem;padding:10px 22px;">🎟 Acheter</a>
                </div>
            <?php endforeach; ?>
            </div>
        ...
        <section class="content-section">
            <h2 class="content-h2">Tarifs</h2>
            <div class="content-grid-2">
                <div class="content-card" style="padding:22px;"><h3>Tribune populaire</h3><div style="font-family:'Bebas Neue',sans-serif;font-size:2rem;color:var(--rouge);margin:6px 0;">10 €</div><p style="color:#666;font-size:0.86rem;">Place debout · Secteur supporter</p></div>
                <div class="content-card" style="padding:22px;"><h3>Tribune principale</h3><div style="font-family:'Bebas Neue',sans-serif;font-size:2rem;color:var(--rouge);margin:6px 0;">15 €</div><p style="color:#666;font-size:0.86rem;">Place assise numérotée</p></div>
                <div class="content-card" style="padding:22px;"><h3>Tarif réduit</h3><div style="font-family:'Bebas Neue',sans-serif;font-size:2rem;color:var(--rouge);margin:6px 0;">7 €</div><p style="color:#666;font-size:0.86rem;">- 18 ans · Seniors · Demandeurs d'emploi</p></div>
                <div class="content-card" style="padding:22px;"><h3>Enfants - 6 ans</h3><div style="font-family:'Bebas Neue',sans-serif;font-size:2rem;color:var(--rouge);margin:6px 0;">Gratuit</div><p style="color:#666;font-size:0.86rem;">Accompagnés d'un adulte payant</p></div>
            </div>
        </section>
        <section class="content-section">
            <h2 class="content-h2">Infos pratiques</h2>
            <div class="content-grid-2">
                <div style="display:flex;align-items:center;gap:14px;padding:16px 20px;background:#fff;border:1px solid #EBEBEB;border-radius:10px;"><span style="font-size:1.6rem;">🎟</span><div><div style="font-size:0.7rem;font-weight:900;letter-spacing:0.12em;text-transform:uppercase;color:#888;margin-bottom:2px;">Guichets</div><div style="font-weight:700;color:var(--navy);">Ouverts 1h30 avant le coup d'envoi</div></div></div>
                <div style="display:flex;align-items:center;gap:14px;padding:16px 20px;background:#fff;border:1px solid #EBEBEB;border-radius:10px;"><span style="font-size:1.6rem;">✉️</span><div><div style="font-size:0.7rem;font-weight:900;letter-spacing:0.12em;text-transform:uppercase;color:#888;margin-bottom:2px;">Contact</div><div style="font-weight:700;color:var(--navy);">tickets@rfcliege.be</div></div></div>
            </div>
        </section>
