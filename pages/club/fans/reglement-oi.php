...
<head>
    ...
    <title>Règlement OI — RFC Liège</title>
...
<section class="page-banner">
    ...
    <div class="page-banner-content">
        <div class="page-breadcrumb">
            ...
            <span class="breadcrumb-sep">›</span>
            <span>Règlement OI</span>
        </div>
        <h1 class="page-h1">Règlement OI</h1>
        <p class="page-desc">Règlement d'ordre intérieur du Stade de Rocourt — RFC Liège.</p>
    </div>
</section>
...
<?php if($cms_r&&$cms_r["contenu"]): ?><main class="page-main"><div class="page-main-inner cms-content"><?= cms_render($cms_r["contenu"]) ?></div></main><?php else: ?><main class="page-main">
    <div class="page-main-inner">
        ...
        <section class="content-section">
            <h2 class="content-h2">Règlement d'ordre intérieur</h2>
            <div class="content-text"><p>Le présent règlement s'applique à toute personne pénétrant dans l'enceinte du Stade de Rocourt lors d'un événement organisé par le RFC Liège. Son respect est obligatoire et conditionne l'accès au stade.</p></div>
        </section>
        <section class="content-section">
            <h2 class="content-h2">Accès au stade</h2>
            <div style="display:flex;flex-direction:column;gap:0;border-radius:10px;overflow:hidden;border:1px solid #EBEBEB;">
                <?php foreach(['Toute personne doit être en possession d'un titre d'accès valide (billet ou abonnement)','Les sacs et effets personnels sont soumis à fouille à l'entrée','L'introduction d'alcool, de substances illicites ou de matériel pyrotechnique est strictement interdite','Il est interdit de pénétrer dans le stade en état d'ivresse manifeste','Les enfants de moins de 14 ans doivent être accompagnés d'un adulte responsable'] as $i=>$r): $bg=$i%2===0?'#fff5f5':'#fff'; ?>
                <div style="display:flex;gap:14px;padding:15px 22px;background:<?= $bg ?>;border-left:4px solid var(--rouge);border-bottom:1px solid #eee;">
                    <span style="color:var(--rouge);font-weight:900;flex-shrink:0;"><?= $i+1 ?>.</span>
                    <span style="font-size:0.94rem;color:var(--navy);"><?= $r ?></span>
                </div>
                <?php endforeach; ?>
            </div>
        </section>
        <section class="content-section">
            <h2 class="content-h2">Comportement dans l'enceinte</h2>
            <div style="display:flex;flex-direction:column;gap:0;border-radius:10px;overflow:hidden;border:1px solid #EBEBEB;">
                <?php foreach(['Respecter les autres spectateurs, le personnel du stade et les forces de l'ordre','Rester dans le secteur indiqué sur son titre d'accès','Ne pas lancer d'objets sur ou en dehors du terrain','Tout comportement violent, discriminatoire ou raciste entraîne l'exclusion immédiate','Respecter les infrastructures et équipements du stade'] as $i=>$r): $bg=$i%2===0?'#fff':'#fff5f5'; ?>
                <div style="display:flex;gap:14px;padding:15px 22px;background:<?= $bg ?>;border-left:4px solid #1aaf5d;border-bottom:1px solid #eee;">
                    <span style="color:#1aaf5d;font-weight:900;flex-shrink:0;">✓</span>
                    <span style="font-size:0.94rem;color:var(--navy);"><?= $r ?></span>
                </div>
                <?php endforeach; ?>
            </div>
